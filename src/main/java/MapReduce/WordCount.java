...
public class WordCount {
    ...
    public static class doMapper extends Mapper<Object, Text, Text, IntWritable> {
        ...
        protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            String PhoneNumber = strList.get(0);
            Data data = new Data(Integer.parseInt(strList.get(-3)), Integer.parseInt(strList.get(-2)));
            //返回当前位置到下一个分隔符之间的字符串
            // word.set(tokenizer.nextToken());

            //将word存到容器中，记一个数
            context.write(new Text(PhoneNumber), data);
        }
    }
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        Path in = new Path("./HTTP_20130313143750.dat");//设置这个作业输入数据的路径(***部分为自己liunx系统的localhost或者ip地址)
        Path out = new Path("./output.txt");        //设置这个作业输出结果的路径
